...
function simulate(currentActivities, proposedChange) {
    ...
    switch (proposedChange.scenario) {
      ...
      case 'reduce_meat':
        ...
        if (copy.category === 'food' && meatTypes.includes(copy.activityType)) {
          const factor =
            typeof proposedChange.reductionFactor === 'number'
              ? proposedChange.reductionFactor
              : 0.5;
      ...
      case 'reduce_ac':
        ...
        if (copy.category === 'energy' && copy.activityType === 'ac_usage') {
          const hoursReduced =
            typeof proposedChange.hoursReduced === 'number' ? proposedChange.hoursReduced : 1;
      ...
      case 'cycle_instead':
        ...
        const maxDist =
          typeof proposedChange.maxDistance === 'number' ? proposedChange.maxDistance : 5;
        if (
          copy.category === 'travel' &&
          motorCommutes.includes(copy.activityType) &&
          copy.value <= maxDist
        ) {
  ...
  const percentageReduction =
    currentTotal > 0 ? ((savedKg / currentTotal) * 100).toFixed(1) : '0.0';
