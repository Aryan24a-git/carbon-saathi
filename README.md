...
|---|---|---|
| **Code Quality** | Structure, readability, maintainability: Zod schema validation, ESLint and Prettier for consistent styling, comprehensive JSDoc on all backend functions, modular architecture, structured JSON logging without `console.log` statements, and robust state verification preventing double-evaluation in the gamified challenge engine. | `server/utils/constants.js`, `server/utils/validators.js`, `.eslintrc.json`, `server/engines/challengeEngine.js` |
| **Security** | Safe and responsible implementation: **Strict Helmet CSP** configured to block unauthorized scripts/styles, **DOMPurify** (client-side bundled copy) implemented across the SPA to mitigate XSS in dynamic rendering, server-side **custom `stripHtml()` sanitizer** for input cleansing, **Zod validation** for payload checking, and **dynamic listener bindings** (zero inline `onclick` attributes in dynamically injected components) to guarantee total CSP compliance. | `server/index.js`, `public/js/dompurify.min.js`, `server/utils/validators.js`, `public/js/challenges.js` |
| **Efficiency** | Optimal use of resources: **Cache-Control** headers for static assets, **version-based cache busting** (`?v=1.0.1`) on client imports, **WebP** image optimization, **deferred** script loading, lightweight CSS 3D (no heavy libraries like Three.js), and 100% local deterministic calculation logic. | `server/index.js`, `public/index.html`, `public/css/style.css` |
| **Testing** | Validation of functionality: 83+ backend test cases covering unit logic, router endpoints, Zod schema sanitization, rate limit headers, and AI API fallback. Reaches 85%+ branch and 99.5% line coverage. | `tests/` |
...
| **SEO & Best Practices** | 100% SEO Lighthouse coverage: Unique title tags, comprehensive Open Graph metadata (`og:title`, `og:description`, `og:url`), `rel="canonical"` referencing, `theme-color` directives, and explicit `robots` configuration. | `public/index.html` |
| **Architecture & AI** | Deterministic local decision engine paired with Gemini AI acting solely as a translation/explanation layer with robust local fallback mechanisms for 100% uptime, and an expanded database of 17 localized college quests. | `server/engines/decisionEngine.js`, `server/services/gemini.js`, `server/engines/challengeEngine.js` |
